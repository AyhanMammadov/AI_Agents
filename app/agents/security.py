...
def security_agent(context: dict) -> dict:
    fallback = {
        "role": "security",
        "deliverables": {},
        "decisions": [],
        "assumptions": [],
        "open_questions": ["Security вернул пустой или некорректный результат"],
        "error": None,
    }
    ...
    try:
        ...
        if not result:
            fallback["error"] = "empty response from security"
            return fallback
        ...
        fallback["error"] = "invalid json from security"
        return fallback

    except Exception as e:
        fallback["error"] = f"security failed: {str(e)}"
        return fallback
